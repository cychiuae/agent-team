...
## How to plan
1. Read the test plan and identify the production-code changes needed to satisfy each TC. Tasks describe production code only — tests are written later, in Phase 4, by `test-writer` against `01-test-plan.md`. Do **not** include test files in any task's `## Files`.
2. Group changes into commit-sized units. Rule of thumb: < ~300 lines diff, single concern, independently committable. The commit must build / type-check / lint cleanly on its own; "test suite green" is not a per-task gate because tests don't exist yet during Phase 3.
...
4. For each task, write:
   ...
   - The TC-# items it is intended to satisfy (traceability only — these will be exercised by tests in Phase 4).
